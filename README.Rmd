...
Patchwork already provides support for quick tag of sub-plots and sub-figures
using `patchwork::plot_annotation()`.
```{r}
pat + plot_annotation(tag_levels = "A")
```

For figures which are all just images, tag _on top_ of the image is 
...
To add borders around indiviual figures, use `b_*` options inside of `figwrap()` 
or also inside `fig()`.
